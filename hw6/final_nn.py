...
'''
This function will set seed values for random
'''
def seed(seed_value):
...
'''
This function will check if GPU support is available
'''
def cuda_available():
...
'''
This function will mount to the google drive folder
'''
def mount_data_gdrive():
  from google.colab import drive
  drive.mount('/content/gdrive', force_remount=True)

'''
This function will transform the data and generate all the data loader
'''

def data_transform_and_loader(input_min_dim, mini_batch_size, valid_percent):
  ...
  #full training dataset - 4000 images, 5 classes; test dataset - 1000 images, 5 classes
  dataset = datasets.ImageFolder(root = "/content/gdrive/My Drive/data/train",transform=data_transforms)
  test_dataset = datasets.ImageFolder(root = "/content/gdrive/My Drive/data/test", transform=data_transforms)

  #Splitting training data as 500 images goes to validation set and rest 3500 to trainig
  dataset_size = len(dataset)-7
  valid_size = int(dataset_size * valid_percent)
  train_size = dataset_size - valid_size

  #permutate the dataset psudorandomly (used seed value for reproducability)
  ...
  train_loader = DataLoader(dataset, pin_memory=True, batch_size=mini_batch_size,
                            sampler=SubsetRandomSampler(train_indices),
                            num_workers = 4)
  valid_loader = DataLoader(dataset, pin_memory=True, batch_size=mini_batch_size,
                            sampler=SubsetRandomSampler(valid_indices),
                            num_workers=4)
  test_loader = DataLoader(test_dataset, pin_memory=True, batch_size=mini_batch_size, 
...
'''
This function will generate the loss function and optimizer
'''

def optimizer_and_loss(model, learning_rate):
...
'''
This function will work as a model selector. With a parameter of 'NN' or 'CNN' we can 
toggle between networks!!
'''

def model_selector(model_name, input_size, num_classes, device):
  if model_name=='NN':
    model = run_neural_network(input_size, num_classes, device)
  elif model_name=='CNN':
    model = run_conv_network(num_classes, device)
  return model

'''
This function will initialize a Fully Connected Neural Network model
'''
def run_neural_network(input_size, num_classes, device):
  model = FCNeuralNet(input_size, num_classes).to(device)
  return model

'''
This function will initialize a Convolutional Neural Network model
'''

def run_conv_network(num_clases, device):
...
if __name__ == "__main__":
  ...
  #load data
  train_loader, valid_loader, test_loader = data_transform_and_loader(input_min_dim=60,
                                                                      mini_batch_size=batch_size, 
                                                                      valid_percent=0.125)

  #model choose
  model = model_selector(model_name=model_name, input_size=input_shape, num_classes=num_classes, 
                         device=device)
  
  #optimizer and loss
  optimizer, loss = optimizer_and_loss(model=model, learning_rate=learning_rate)
  ...
  for epoch in range(num_epochs):
    ...
    print(f'Epoch[{epoch+1}/{num_epochs}] Training Accuracy: {training_accuracy:.4f} Training Loss {training_loss:.4f} Validation Accuracy: {validation_accuracy:.4f} Validation Loss: {validation_loss:.4f}')
...
print(
        f'Training Loss: {global_tr_loss:.4f} Training Accuracy: {global_tr_acc} Validation Accuracy: {global_v_acc} Validation Loss: {global_v_loss} Test Accuracy: {test_accuracy}')
